...
4. `${CLAUDE_PLUGIN_ROOT}/references/cpq-salesforce-fields.json` — valid `zqu__Quote__c`, `zqu__QuoteRatePlanCharge__c`, and tier field names for patches and reads
...
**QRPC field resolution:** When the user references a UI label such as "QRPC.Effective Price" or "Effective Price", resolve the API name from `cpq-salesforce-fields.json` (`zqu__EffectivePrice__c` on `zqu__QuoteRatePlanCharge__c`) before generating code. Do not guess field names from labels. Read charge metrics from `charge.record.<FieldApiName>`; tier effective price uses `zqu__Effective_Price__c` on `zqu__QuoteCharge_Tier__c`, not on QRPC. Run `lint-cpq-hooks-events.js` — unknown QRPC/tier fields fail with `WJS082`, and known bad names fail with `EJS090`.
